...
namespace Kazetta
{
	public partial class MainWindow : System.Windows.Window
	{
		...
		private void LoadXML()
		{
			...
			// The improv lesson for everyone should be one slot after the solfeggio lesson.
			if (viewModel.AppData.Students.Any() && viewModel.AppData.Teachers.Any())
			{
				viewModel.InitSchedule();
				Algorithms.ExtraPostprocess(viewModel);
            }
		...
		private void AddGroup(object sender, RoutedEventArgs e)
		{
			...
			var persons = group.Persons.ToList().FindAll(person => person != null);
			if (persons.Count < 2)
			{
				return;
			}
			group.Persons = persons.ToArray();

			viewModel.AddGroup(group);
			viewModel.NewGroup = new Group { Autogenerated = false, Persons = new Student[3] };
		}
		...
		private void AutoCreateGroups(object sender, RoutedEventArgs e)
		{
			...
			while (groupEligible.Count > 1)
			{
				...
				if (p2 != null)
				{
                    ...
                    if (groupEligible.Count > 2 * (viewModel.NUM_LESSONS - viewModel.Groups.Count(gg => gg.Persons.Length > 1)))
					{
