...
namespace Kazetta
{
	public partial class MainWindow : System.Windows.Window
	{
		...
		{
			InitializeComponent();
			kcs = new DnDItemsControl[] { kcs1, kcs2, kcs3, kcs4, kcs5, kcs6, kcs7, kcs8, kcs9, kcs10, kcs11, kcs12, kcs13, kcs14, kcs15, kcs16, kcs17, kcs18, kcs19, kcs20 };
		...
		private void NewGroup_SelectionChanged(object sender, SelectionChangedEventArgs e)
		{
			if (e.AddedItems.Count > 0)
			{
				...
				PossiblePairs.ItemsSource = viewModel.GroupEligible.Cast<Student>().Where(q => p != q && q.Pair == null && p.Instrument == q.Instrument);
			}
			else
		...
		{
			var groupEligible = viewModel.GroupEligible.Cast<Student>().Where(p => !(p.Instrument == Instrument.Guitar && p.SkillLevel == Level.Advanced)).ToList();
			while (groupEligible.Count > 1)
			{
				Student p = groupEligible.First();
				Student q = groupEligible.FirstOrDefault(r => p != r && r.Pair == null && p.Instrument == r.Instrument);
				if (q != null)
				{
					Group g = new Group { Autogenerated = false, Persons = new Student[] { p, q } };
					viewModel.AddGroup(g);
					groupEligible.Remove(p);
					groupEligible.Remove(q);
				}
			}
		}
	}
